        ...
        <div class="card shadow-sm mb-4">
            <div class="card-body">
                <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
                    ...
                    @php
                        $selectedRating = $viewData['filters']['rating'] ?? null;
                    @endphp
                    <a href="{{ route('admin.review.index') }}" class="btn btn-sm {{ $selectedRating === null ? 'btn-primary' : 'btn-outline-primary' }}">
                        {{ __('admin.reviews.filters.all') }}
                    </a>
                    @for ($i = 5; $i >= 1; $i--)
                        <a href="{{ route('admin.review.index', array_merge(request()->query(), ['rating' => $i])) }}"
                           class="btn btn-sm {{ (int) $selectedRating === $i ? 'btn-primary' : 'btn-outline-primary' }}">
                        ...
                        <tbody>
                            @forelse ($viewData['reviews'] as $review)
                                <tr>
                                    ...
                                    <td>{{ $review->getProduct()?->getName() ?? __('admin.orders.show.product_unavailable') }}</td>
                                    ...
                                    <td style="max-width: 320px;">
                                        <span class="text-muted">{{ \Illuminate\Support\Str::limit($review->getDescription(), 100) }}</span>
                                    </td>
                                    ...
                                    <td class="text-end">
                                        <form action="{{ route('admin.review.destroy', $review->getId()) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('{{ __('admin.reviews.actions.confirm_delete') }}')">
